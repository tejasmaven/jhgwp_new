...
abstract class Field implements FieldInterface {
    ...
    public function import($importData, $args = array()){
	    ...
	    if ($this->parsingData['logger']) {
		    call_user_func(
			    $this->parsingData['logger'],
			    sprintf(
			        // Translators: %1$s is the container name concatenated with the field name.
				    esc_html__('- Importing field `%1$s`', 'csv-xml-import-for-acf'),
				    esc_html($this->importData['container_name'] . $field['name'])
			    )
		    );
	    }
        ...
        if (!empty($this->importData['articleData']['ID']) && ! \pmai_is_acf_update_allowed($this->importData['container_name'] . $field['name'], $this->parsingData['import']->options, $this->parsingData['import']->id)) {
            $this->parsingData['logger'] && call_user_func($this->parsingData['logger'],
	            // Translators: %1$s is the name of the field.
	            sprintf(__('- Field `%1$s` is skipped attempted to import options', 'csv-xml-import-for-acf'), $this->getFieldName()));
            return FALSE;
        }
    ...
    /**
     * @return \pmai_acf_add_on\fields\Field|bool
     */
    ...
    /**
     * @param \pmai_acf_add_on\fields\Field|bool $parent
     */
